                ...
                <div className="max-w-5xl pl-8 basis-3/4 flex-1">
                            ...
                            <form onSubmit={handleSubmit} className="flex flex-col gap-6 mt-6">
                                <div className="flex gap-4 w-full">
                                    <div className="flex w-full flex-col gap-2">
                                        ...
                                        <TextInput className="w-full" id="title" value={data.title} onChange={e => setData('title', e.target.value)}/>
                                    </div>
                                    <div className="flex w-full flex-col gap-2">
                                        <InputLabel htmlFor="max_user" value={"Max User"}/>
                                        <TextInput type={"number"} className="w-full" id="max_user" value={data.max_user} onChange={e => setData('max_user', e.target.value)}/>
                                    </div>
                                </div>
                                <div className="flex flex-col gap-2">
                                    <InputLabel htmlFor="description" value={"Group Description"} />
                                    <TextArea className="w-full" id="description" value={data.description} onChange={e => setData('description', e.target.value)}/>
                                </div>
                    ...
                    <div className="flex flex-col gap-6">
                        { groups && groups.map((group) => (
                            <GroupAuthCard key={group?.id} title={group?.title} />
                        ))}
                    </div>
                </div>
