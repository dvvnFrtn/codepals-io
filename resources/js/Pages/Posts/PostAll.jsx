                ...
                <div className="max-w-5xl pl-8 basis-3/4 flex-1">
                    <div className="flex flex-col w-full bg-white rounded-xl shadow-sm p-6 mt-16 mb-8">
                        <div className="flex flex-row justify-between items-center">
                            <h1 className="font-semibold text-2xl text-slate-800">All Posts</h1>
                        </div>
                        <form onSubmit={handleSubmit} className="mt-8" encType="multipart/form-data">
                            <div className="flex flex-col gap-2">
                                <InputLabel htmlFor="content" value={"Whats on your mind"} />
                                <TextArea className="w-full" id="content" name="content" value={data.content} onChange={(e) => setData('content', e.target.value)} />
                            </div>
                            <div className="flex justify-between items-center mt-4">
                                <TextInput type="file" name="image" id="image" className="hidden" onChange={handleImageChange}/>
                                <label htmlFor="image" id="lb-image" className="cursor-pointer">
                    ...
                    <div className="flex flex-col gap-6">
                        { posts && posts.map((post) => (
                            <PostCard key={post?.id} username={post?.user.name} body={post?.content} updated_at={post?.formatted_updated_at} image_path={post?.image_path} picture={post?.user.picture}/>
                        ))}
                    </div>
                </div>
